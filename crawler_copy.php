...
foreach ($productElements as $productElement) {
   ...
   // Make additional HTTP request to get description from the product detail page
    $productDetailUrl = $titleElement ? $titleElement->href : null;

    if ($productDetailUrl) {
        $productDetailContent = file_get_contents($productDetailUrl, false, $context);

    $productDetailUrl = strpos($productDetailUrl, 'http') === 0 ? $productDetailUrl : rtrim($url, '/') . '/' . ltrim($productDetailUrl, '/');
    ...
    $productDetailContent = file_get_contents($productDetailUrl, false, $context);
        ...
        if ($productDetailContent !== false) {
            ...
            $descriptionElement = $productDetailHtml->find('div.product__description > product__description--wrap > ul > li', 0);
